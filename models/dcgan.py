...
from keras.layers import Input, Dense, Flatten, Reshape, BatchNormalization, Dropout, Activation,LeakyReLU, UpSampling2D, Conv2D, ZeroPadding2D
...
class GAN():
    ...
    def build_generator(self):
        ...
        model.add(Dense(128 * 8 * 8, input_dim=self.latent_dim, activation='relu'))
        model.add(Reshape((8, 8, 128)))
        model.add(UpSampling2D()) # upsampled to 16,16,128
        model.add(Conv2D(128, kernel_size=3, padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Activation('relu'))
        model.add(UpSampling2D()) # upsampled to 32,32,128
        model.add(Conv2D(64, kernel_size=3, padding='same'))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Activation('relu'))
        model.add(Conv2D(self.channels, kernel_size=3, padding='same'))
        model.add(Activation('tanh'))
        model.add(UpSampling2D(size=(2,2))) # upsampled to 64,64,1
    ...
    def build_discriminator(self):
        ...
        model.add(ZeroPadding2D(padding=((0, 1), (0, 1)))) # adds padding as rows and columns of 0s
        model.add(BatchNormalization(momentum=0.8))
        model.add(LeakyReLU(0.2))
        model.add(Dropout(0.25))
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                ax[i, j].axis('off')
                cnt = cnt + 1
        fig.savefig(f'../generated_images_dcgan/{epoch}.jpg')
        plt.close()
    ...
    def plot_losses_and_accuracy(self, epochs):
        ...
        fig.tight_layout()
        fig.savefig('../generated_images_dcgan/losses_and_accuracy_dcgan.jpg')
        plt.close(fig)
